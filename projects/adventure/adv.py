...
def explore_depths(previous, last_move, graph):
    ...
    if '?' in graph[current].values():
        ...
        explore_depths(current, direction, graph)
        # print(direction)
        # for direction in graph[current]:
            # wrong, needs to be not every direction or this immediately solves the graph, inefficiently. probably needs to be a random ? direction
            # if graph[current][direction] is '?':
                # player.travel(direction)
                # explore_depths(current, direction, graph)
    else:
        print(graph)
...
def nearest_unexplored(starting, graph):
    ...
    while q.size > 0:
        current = q.dequeue()
        for next in graph[current].values():
            if next not in path:
                q.enqueue(next)
...
def adventure():
    ...
    while True:
        explore_depths(previous, 'n', graph)
        print(graph)
        current = player.currentRoom.id
        nearest_unexplored(current, graph)
        if '?' not in graph.values(): return graph
        # not sure this is the right way to break the loop, depends on how graph is updated
